...
public static class AllQuests
{
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "berrybush" && action == "complete_harvest",
		QuestGroup = QuestGroup.Starter,
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "rock" && action == "complete_harvest",
		QuestGroup = QuestGroup.Starter
	};
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "tree" && action == "complete_harvest",
		QuestGroup = QuestGroup.Starter
	};

	public static Quest CarveAStick = new Quest("carve_stick")
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_carving" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Starter
	};

	public static Quest PauseGame = new Quest("pause_game")
	{
		OnSpecialAction = (string action) => action == "pause_game" && QuestManager.instance.QuestIsComplete("carve_stick"),
		QuestGroup = QuestGroup.Starter
	};

	public static Quest PlantBerryBush = new Quest("plant_berry_bush")
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_growth" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Starter
	};

	public static Quest BuildAHouse = new Quest("build_house")
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_house" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Starter,
		IsSteamAchievement = true
	};
	...
	{
		OnCardCreate = (CardData card) => WorldManager.instance.CardQuery.GetCardCount<BaseVillager>() == 2,
		QuestGroup = QuestGroup.Starter,
		IsSteamAchievement = true
	};
	...
	{
		OnCardCreate = (CardData card) => WorldManager.instance.CardQuery.GetCardCount<House>() >= 3,
	...
	{
		OnActionComplete = (CardData card, string action) => action == "complete_harvest" && card.Id == "forest",
		QuestGroup = QuestGroup.Exploration
	};

	public static Quest ExploreAMountain = new Quest("explore_mountain")
	{
		OnActionComplete = (CardData card, string action) => action == "complete_harvest" && card.Id == "mountain",
		QuestGroup = QuestGroup.Exploration
	};
	...
	{
		OnCardCreate = (CardData card) => WorldManager.instance.GetFoodCount(false) >= 5,
		DefaultVisible = true,
		QuestGroup = QuestGroup.Resources,
	...
	{
		OnCardCreate = (CardData card) => WorldManager.instance.GetFoodCount(false) >= 10,
		QuestGroup = QuestGroup.Resources,
		DescriptionTermOverride = "quest_have_food_text",
		RequiredCount = 10
	};
	...
	{
		OnCardCreate = (CardData card) => WorldManager.instance.GetFoodCount(false) >= 20,
		QuestGroup = QuestGroup.Resources,
		DescriptionTermOverride = "quest_have_food_text",
		RequiredCount = 20
	};
	...
	{
		OnCardCreate = (CardData card) => WorldManager.instance.GetFoodCount(false) >= 50,
		QuestGroup = QuestGroup.Resources,
		DescriptionTermOverride = "quest_have_food_text",
		RequiredCount = 50
	};
	...
	{
		OnSpecialAction = (string action) => action == "month_end" && WorldManager.instance.Time.CurrentMonth >= 6,
	...
	{
		OnSpecialAction = (string action) => action == "month_end" && WorldManager.instance.Time.CurrentMonth >= 12,
		QuestGroup = QuestGroup.Survival,
		DescriptionTermOverride = "quest_reach_month_text",
		RequiredCount = 12
	};
	...
	{
		OnSpecialAction = (string action) => action == "month_end" && WorldManager.instance.Time.CurrentMonth >= 24,
		QuestGroup = QuestGroup.Survival,
		IsSteamAchievement = true,
		DescriptionTermOverride = "quest_reach_month_text",
		RequiredCount = 24
	...
	{
		OnSpecialAction = (string action) => action == "month_end" && WorldManager.instance.Time.CurrentMonth >= 36,
		QuestGroup = QuestGroup.Survival,
		DescriptionTermOverride = "quest_reach_month_text",
		RequiredCount = 36
	};
	...
	{
		OnCardCreate = (CardData card) => WorldManager.instance.CardQuery.GetCardCount<BaseVillager>() == 3,
		QuestGroup = QuestGroup.MainQuest
	};
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_temple" && action == "finish_blueprint",
		QuestGroup = QuestGroup.MainQuest
	};
	...
	{
		OnCardCreate = (CardData card) => WorldManager.instance.Economy.GetGoldCount(true) >= 10,
		QuestGroup = QuestGroup.Resources,
		DefaultVisible = true,
	...
	{
		OnCardCreate = (CardData card) => WorldManager.instance.Economy.GetGoldCount(true) >= 30,
		QuestGroup = QuestGroup.Resources,
		DescriptionTermOverride = "quest_have_gold_text",
		RequiredCount = 30
	};
	...
	{
		OnCardCreate = (CardData card) => WorldManager.instance.Economy.GetGoldCount(true) >= 50,
		QuestGroup = QuestGroup.Resources,
		IsSteamAchievement = true,
		DescriptionTermOverride = "quest_have_gold_text",
		RequiredCount = 50
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "driftwood" && action == "complete_harvest",
		QuestGroup = QuestGroup.Island_Beginnings
	};

	public static Quest Have3Shells = new Quest("have_3_shells")
	{
		OnCardCreate = (CardData card) => WorldManager.instance.Economy.GetShellCount(true) >= 3,
		QuestGroup = QuestGroup.Island_Beginnings,
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_fish_trap" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Island_Beginnings
	};

	public static Quest MakeSail = new Quest("make_sail")
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_sail" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Island_Beginnings
	};

	public static Quest BuildSloop = new Quest("build_sloop")
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_sloop" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Island_Beginnings,
		IsSteamAchievement = true
	};
	...
	{
		OnCardCreate = (CardData card) => WorldManager.instance.Economy.GetShellCount(true) >= 10,
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_mess_hall" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Island_Misc
	};

	public static Quest BuildGreenhouse = new Quest("build_greenhouse")
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_greenhouse" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Island_Misc,
		IsSteamAchievement = true
	};
	...
	{
		OnActionComplete = (CardData card, string action) => (card.Id == "blueprint_sushi" || card.Id == "blueprint_tamago_sushi") && action == "finish_blueprint",
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_ceviche" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Island_Cooking,
		IsSteamAchievement = true
	};
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_seafood_stew" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Island_Cooking
	};

	public static Quest MakeRum = new Quest("make_rum")
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_rum" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Island_Cooking,
		IsSteamAchievement = true
	};
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_glass" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Island_Misc
	};
	...
	{
		OnSpecialAction = (string action) => action == "completed_forest_wave" && WorldManager.instance.CurrentRunVariables.ForestWave == 1,
		QuestGroup = QuestGroup.Forest_MainQuest
	};

	public static Quest BuildStablePortal = new Quest("build_stable_portal")
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_stable_portal" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Forest_MainQuest,
		IsSteamAchievement = true
	};
	...
	{
		OnSpecialAction = (string action) => action == "completed_forest_wave" && WorldManager.instance.CurrentRunVariables.ForestWave == 6,
		QuestGroup = QuestGroup.Forest_MainQuest,
		IsSteamAchievement = true
	};
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "archer" && action == "equip_item",
	...
	{
		OnActionComplete = (CardData card, string action) => card.GetAllEquipables().Any<Equipable>((Equipable card) => card.Id == "bunny_hat") && action == "equip_item",
		QuestGroup = QuestGroup.Equipment,
		IsSteamAchievement = true
	};
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_smithy" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Equipment,
		IsSteamAchievement = true
	};
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_broken_bottle" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Equipment
	};

	public static Quest WearCrabScaleArmor = new Quest("wear_crab_scale_armor")
	{
		OnActionComplete = (CardData card, string action) => card.GetAllEquipables().Any<Equipable>((Equipable card) => card.Id == "crab_scale_armor") && action == "equip_item",
		QuestGroup = QuestGroup.Equipment
	};

	public static Quest TrainWizard = new Quest("train_wizard")
	{
		OnActionComplete = (CardData card, string action) => card.Id == "wizard" && action == "equip_item",
		QuestGroup = QuestGroup.Equipment
	};

	public static Quest CraftAmuletForest = new Quest("craft_amulet_forest")
	{
		OnActionComplete = (CardData card, string action) => card.Id == "blueprint_amulet_of_forest" && action == "finish_blueprint",
		QuestGroup = QuestGroup.Equipment
	};
	...
	public static Quest EquipArcherQuiver = new Quest("equip_archer_quiver")
	{
		OnActionComplete = delegate(CardData card, string action)
		{
			if (card.Id == "archer")
			{
				if (card.GetAllEquipables().Any<Equipable>((Equipable card) => card.Id == "quiver") && action == "equip_item")
				{
					return true;
				}
			}
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "ninja" && action == "equip_item",
		QuestGroup = QuestGroup.Equipment
	};
	...
	{
		OnCardCreate = (CardData card) => WorldManager.instance.CardQuery.GetCardCount<Poop>() >= 3,
		QuestGroup = QuestGroup.Death_Starter
	};
	...
	{
		OnCardCreate = (CardData card) => WorldManager.instance.CardQuery.GetCardCount<Happiness>() >= 2,
		QuestGroup = QuestGroup.Happiness_Starter
	};
	...
	{
		OnCardCreate = (CardData card) => WorldManager.instance.CardQuery.GetCardCount<Unhappiness>() >= 1 && card.MyGameCard.MyBoard.Id == "happiness",
		QuestGroup = QuestGroup.Happiness_Unhappy
	};

	public static Quest Have5Unhappiness = new Quest("happiness_have_5_unhappiness")
	{
		OnCardCreate = (CardData card) => WorldManager.instance.CardQuery.GetCardCount<Unhappiness>() >= 5 && card.MyGameCard.MyBoard.Id == "happiness",
		QuestGroup = QuestGroup.Happiness_Unhappy
	};

	public static Quest Have10Unhappiness = new Quest("happiness_have_10_unhappiness")
	{
		OnCardCreate = (CardData card) => WorldManager.instance.CardQuery.GetCardCount<Unhappiness>() >= 10 && card.MyGameCard.MyBoard.Id == "happiness",
		QuestGroup = QuestGroup.Happiness_Unhappy
	};
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "trash" && action == "complete_harvest",
		QuestGroup = QuestGroup.Cities_Starter
	};
	...
	{
		OnActionComplete = (CardData card, string action) => card.Id == "farmland" && action == "harvest",
		QuestGroup = QuestGroup.Cities_Starter
	};
	...
	public static Quest CitiesBuildLandmark = new Quest("cities_build_second_landmark")
	{
		OnActionComplete = delegate(CardData card, string action)
		{
			...
			if (blueprint != null)
			{
				...
				Subprint subprint = blueprint.Subprints.FirstOrDefault<Subprint>();
				return list2.Contains((subprint != null) ? subprint.ResultCard : null) && card.Id != "blueprint_park" && action == "finish_blueprint";
			}
			return false;
		},
	...
	public static Quest CitiesAllLandmarks = new Quest("cities_all_landmarks")
	{
		OnActionComplete = delegate(CardData card, string action)
		{
			List<string> list3 = (from x in WorldManager.instance.GameDataLoader.CardDataPrefabs
				where x is Landmark
				select x.Id).ToList<string>();
			list3.Remove("particle_collider");
			list3.Remove("time_machine");
			List<string> list4 = (from x in WorldManager.instance.CardQuery.GetAllCardsOnBoard("cities")
				where x.CardData is Landmark
				select x.CardData.Id).ToList<string>();
	...
	{
		OnCardCreate = (CardData card) => card is Worker && WorldManager.instance.CardQuery.GetCardCount<Worker>() >= 6,
		QuestGroup = QuestGroup.Cities_Worker
	};

	public static Quest Cities12Workers = new Quest("cities_12_workers")
	{
		OnCardCreate = (CardData card) => card is Worker && WorldManager.instance.CardQuery.GetCardCount<Worker>() >= 12,
		QuestGroup = QuestGroup.Cities_Worker
	};
	...
	public static Quest CitiesConnectTransport = new Quest("cities_transport")
	{
		OnSpecialAction = delegate(string action)
		{
			if (action == "cities_cable_connected")
			{
				return WorldManager.instance.CardQuery.GetAllCardsOnBoard(WorldManager.instance.CurrentBoard.Id).Sum<GameCard>((GameCard x) => x.CardConnectorChildren.Count<CardConnector>((CardConnector x) => x.ConnectionType == ConnectionType.Transport && x.ConnectedNode != null)) >= 1;
			}
			return false;
		},
		QuestGroup = QuestGroup.Cities_Automation
	...
	public static Quest CitiesConnectTransport5 = new Quest("cities_transport_5")
	{
		OnSpecialAction = delegate(string action)
		{
			if (action == "cities_cable_connected")
			{
				return WorldManager.instance.CardQuery.GetAllCardsOnBoard(WorldManager.instance.CurrentBoard.Id).Sum<GameCard>((GameCard x) => x.CardConnectorChildren.Count<CardConnector>((CardConnector x) => x.ConnectionType == ConnectionType.Transport && x.CardDirection == CardDirection.input && x.ConnectedNode != null)) >= 5;
			}
			return false;
		},
		QuestGroup = QuestGroup.Cities_Automation
	...
	public static Quest CitiesFirstCable = new Quest("cities_first_cable")
	{
		OnSpecialAction = delegate(string action)
		{
			if (action == "cities_cable_connected")
			{
				return WorldManager.instance.CardQuery.GetAllCardsOnBoard(WorldManager.instance.CurrentBoard.Id).Sum<GameCard>((GameCard x) => x.CardConnectorChildren.Count<CardConnector>((CardConnector x) => (x.ConnectionType == ConnectionType.LV || x.ConnectionType == ConnectionType.HV) && x.ConnectedNode != null)) >= 1;
			}
			return false;
		},
	...
	public static Quest Cities5LowPower = new Quest("cities_5_low_power")
	{
		OnSpecialAction = delegate(string action)
		{
			if (action == "cities_cable_connected")
			{
				return WorldManager.instance.CardQuery.GetAllCardsOnBoard(WorldManager.instance.CurrentBoard.Id).Sum<GameCard>((GameCard x) => x.CardConnectorChildren.Count<CardConnector>((CardConnector x) => x.ConnectionType == ConnectionType.LV && x.CardDirection == CardDirection.input && x.ConnectedNode != null)) >= 5;
			}
			return false;
		},
		QuestGroup = QuestGroup.Cities_Power
	...
	public static Quest Cities10LowPower = new Quest("cities_10_low_power")
	{
		OnSpecialAction = delegate(string action)
		{
			if (action == "cities_cable_connected")
			{
				return WorldManager.instance.CardQuery.GetAllCardsOnBoard(WorldManager.instance.CurrentBoard.Id).Sum<GameCard>((GameCard x) => x.CardConnectorChildren.Count<CardConnector>((CardConnector x) => x.ConnectionType == ConnectionType.LV && x.CardDirection == CardDirection.input && x.ConnectedNode != null)) >= 10;
			}
			return false;
		},
		QuestGroup = QuestGroup.Cities_Power
	...
	public static Quest Cities5HighPower = new Quest("cities_high_power")
	{
		OnSpecialAction = delegate(string action)
		{
			if (action == "cities_cable_connected")
			{
				return WorldManager.instance.CardQuery.GetAllCardsOnBoard(WorldManager.instance.CurrentBoard.Id).Any<GameCard>((GameCard x) => x.CardConnectorChildren.Any<CardConnector>((CardConnector x) => x.ConnectionType == ConnectionType.HV && x.ConnectedNode != null));
			}
			return false;
		},
		QuestGroup = QuestGroup.Cities_Power
	...
	public static Quest Cities10HighPower = new Quest("cities_5_high_power")
	{
		OnSpecialAction = delegate(string action)
		{
			if (action == "cities_cable_connected")
			{
				return WorldManager.instance.CardQuery.GetAllCardsOnBoard(WorldManager.instance.CurrentBoard.Id).Sum<GameCard>((GameCard x) => x.CardConnectorChildren.Count<CardConnector>((CardConnector x) => x.ConnectionType == ConnectionType.HV && x.CardDirection == CardDirection.input && x.ConnectedNode != null)) >= 5;
			}
			return false;
		},
		QuestGroup = QuestGroup.Cities_Power
	...
	public static Quest CitiesGreenEnergy = new Quest("cities_green_energy")
	{
		OnSpecialAction = delegate(string action)
		{
			...
			Func<CardConnector, bool> <>9__244;
			return action == "cities_cable_connected" && WorldManager.instance.CardQuery.GetAllCardsOnBoard(WorldManager.instance.CurrentBoard.Id).Sum<GameCard>(delegate(GameCard x)
			{
				IEnumerable<CardConnector> cardConnectorChildren = x.CardConnectorChildren;
				Func<CardConnector, bool> func;
				if ((func = <>9__244) == null)
				{
					func = (<>9__244 = (CardConnector x) => (x.ConnectionType == ConnectionType.LV || x.ConnectionType == ConnectionType.HV) && x.ConnectedNode != null && greenConnectors.Contains(x.ConnectedNode.Parent.CardData.Id));
				}
				return cardConnectorChildren.Count<CardConnector>(func);
			}) >= 10;
		},
		QuestGroup = QuestGroup.Cities_Misc
	};
