...
public class ModDisablingScreen : MewtationsScreen
{
	private void Awake()
	{
		...
		{
			list.AddRange(ModManager.LoadedMods.Select<Mod, ModManifest>((Mod m) => m.Manifest));
		}
		...
		using (List<ModManifest>.Enumerator enumerator = list.GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				ModManifest manifest = enumerator.Current;
