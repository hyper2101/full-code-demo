...
namespace Mewtations.Combat.Battlefield
{
	public class BattlefieldContext
	{
		...
		private void JoinWithConflict(BattlefieldContext otherConflict)
		{
		...
		private bool OverlapsWith(BattlefieldContext otherConflict)
		{
			Bounds bounds = this.GetBounds();
		...
		// =========================================================================
		// TACTICAL RULESPACE & THREAT PROJECTION (CombatV2)
		// =========================================================================
		public class TacticalCell
		{
			public int SlotIndex;
			public bool IsPlayerSide;
			public Combatable Occupant;
			public bool IsCorpse;
		}

		public bool IsCellOccupied(int slotIndex, bool isPlayerSide)
		{
			// A cell is occupied if there's a participant mapped to this slot that is alive or currently a corpse in DeathResolution.
			foreach (var p in Participants)
			{
				if (p == null) continue;
				bool pIsPlayer = (p.Team == Team.Player);
				if (pIsPlayer == isPlayerSide)
				{
					int pSlot = GetParticipantSlotIndex(p);
					if (pSlot == slotIndex && (p.HealthPoints > 0 || p.CurrentHitText != null))
					{
						return true;
					}
				}
		...
		private int GetParticipantSlotIndex(Combatable p)
		{
			return GetIndexInTeam(p);
		}

		public bool IsProtectedCell(int slotIndex, bool isPlayerSide)
		{
			// Backline cells (3, 4, 5) are protected if their corresponding frontline cell (0, 1, 2) is occupied by an ally.
			if (slotIndex < 3 || slotIndex > 5) return false;
			int frontlineSlot = slotIndex - 3;
			return IsCellOccupied(frontlineSlot, isPlayerSide);
		}

		public bool IsControlZone(int slotIndex, bool isPlayerSide)
		{
			// Control Zone is the frontline (0, 1, 2) when occupied by active combatants.
			if (slotIndex < 0 || slotIndex > 2) return false;
			return IsCellOccupied(slotIndex, isPlayerSide);
		}

		public List<int> GetThreatZone(int slotIndex)
		{
			// Adjacent cells around a mutant/aggressive entity (up, down, left, right on 2x3 grid)
			List<int> adjacent = new List<int>();
			int col = slotIndex % 3;
			int row = slotIndex / 3;

			if (col > 0) adjacent.Add(slotIndex - 1);
			if (col < 2) adjacent.Add(slotIndex + 1);
			if (row > 0) adjacent.Add(slotIndex - 3);
			if (row < 1) adjacent.Add(slotIndex + 3);
